...
function New-PostgreSQLServer {
    ...
    # تحويل كلمة المرور الآمنة إلى نص عادي للاستخدام مع أوامر Azure CLI
    ...
    $connectionString = "postgres://$AdminUser:$PasswordText@$ServerName.postgres.database.azure.com:5432/$DBName"
...
function Main {
    ...
    # التحقق من وجود مجموعة الموارد
    Test-ResourceGroup -ResourceGroup $ResourceGroup -Location $Location
    ...
    if ($null -eq $postgresInfo) {
        Write-Host "فشل في إنشاء خادم PostgreSQL. توقف العملية." -ForegroundColor Red
        return
    }
    ...
    if ($null -eq $cosmosInfo) {
        Write-Host "فشل في إنشاء حساب Cosmos DB. توقف العملية." -ForegroundColor Red
        return
    }
    ...
    # إظهار معلومات عن المستودعات والخدمات التي تم إنشاؤها
    Write-Host "`n===== المستودعات والخدمات المدعومة =====" -ForegroundColor Cyan
    Write-Host "المستودعات:" -ForegroundColor White
    Write-Host "- ModelFallbackLogRepository: لتتبع عمليات التبديل بين النماذج" -ForegroundColor White
    Write-Host "- AiModelConfigRepository: لإدارة إعدادات نماذج الذكاء الاصطناعي" -ForegroundColor White
    Write-Host "- PromptTemplateRepository: لإدارة قوالب البرومبت" -ForegroundColor White
    Write-Host "- PromptLibraryImportRepository: لإدارة عمليات استيراد مكتبة البرومبت" -ForegroundColor White
    Write-Host "- ModelUsageStatsRepository: لإدارة إحصائيات استخدام النماذج" -ForegroundColor White
    Write-Host "- AiTeacherRepository: لإدارة المعلمين الذكيين" -ForegroundColor White
    
    Write-Host "`nالخدمات:" -ForegroundColor White
    Write-Host "- ApiKeyService: لإدارة مفاتيح API" -ForegroundColor White
    Write-Host "- CryptoService: للتشفير وفك التشفير" -ForegroundColor White
    Write-Host "- SystemSettingService: لإدارة إعدادات النظام" -ForegroundColor White
    Write-Host "- AiModelConfigService: لإدارة إعدادات نماذج الذكاء الاصطناعي" -ForegroundColor White
    Write-Host "- PromptTemplateService: لإدارة قوالب البرومبت" -ForegroundColor White
    Write-Host "- ModelUsageStatsService: لإدارة إحصائيات استخدام النماذج" -ForegroundColor White
}
